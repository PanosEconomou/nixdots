...
{ config, lib, pkgs, ... }:
let
  cfg = config.pantry.system.hardware.laptop_power;
...
in
{
  options.pantry.system.hardware.laptop_power = {
    enable = lib.mkEnableOption "enable laptop power management tools";
  };
